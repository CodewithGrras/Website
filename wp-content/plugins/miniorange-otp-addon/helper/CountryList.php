...
class CountryList
{
    ...
    public static $countries = array(
		...
		array ( 'name' => 'Vatican City (Città del Vaticano)','alphacode' => 'va', 'countryCode' => '+379') ,
    ...
    {
        return !MoUtility::isBlank(get_mo_gf_option('default_country_code'))
                    ? get_mo_gf_option('default_country_code') : null;
    }
    ...
    {
        return !MoUtility::isBlank(get_mo_gf_option('default_country_code'))
                    && $value === get_mo_gf_option('default_country_code');
	}
	...
	{
		return !MoUtility::isBlank(get_mo_gf_option('default_country'))
					? maybe_unserialize(get_mo_gf_option('default_country')) : null;
	}
	...
	public static function getDefaultCountryCode()
	{
		...
		return !MoUtility::isBlank($oldValue) ? $oldValue : ( !MoUtility::isBlank($newValue) ? $newValue['countryCode'] : null );
	}
	...
	public static function isCountrySelected($value,$alphacode)
	{
		...
		$newValue = self::getDefaultCountryData();
		return $oldValue ? $oldValue : (!MoUtility::isBlank($newValue) && $alphacode == $newValue['alphacode']);
	}
	...
	public static function getDefaultCountryIsoCode()
	{
		$oldValue = self::getDefaultCountryCode_deprecated();
		$newValue = self::getDefaultCountryData();
		if(!MoUtility::isBlank($newValue)) return $newValue['alphacode'];
		if(!MoUtility::isBlank($oldValue)){
			foreach (self::$countries as $country) {
				if($oldValue==$country['countryCode']) return $country['alphacode'];
			}
		}
		return '';
	}
	...
	public static function getOnlyCountryList()
	{
		return null;
	}
}